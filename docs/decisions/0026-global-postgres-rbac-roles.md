...
every tenant; the difference between one tenant's `<id>_app` and another's is
nothing.
...
`NOLOGIN`, no role attributes, ensured idempotently by `mksrv tenant apply`
(`ensureGlobalDBRoles`, once per run, before the per-tenant loop) — so this also
works for the standalone `database` stack, which has no bootstrap step:
...
Nothing has run against these databases. Rollout is: drop every `db_<id>` and
the old `<id>` / `<id>_{app,web,anon}` roles on the Patroni primary, then
...
- Touchpoints: `internal/cli/database.go` (`ensureGlobalDBRoles`, slimmed
  `tenantDatabaseSQL`, constant pre-request SQL, pgAdmin username),
  `stacks/database/templates/postgrest.container.tmpl` (`PGRST_DB_ANON_ROLE`),
  `internal/cli/tenant.go` (the `role` claim), `internal/cli/openbao_tenant.go`
  (mirrored `username` → `<id>_login`), `docs/rbac.md`, ADR 0016 (superseded
